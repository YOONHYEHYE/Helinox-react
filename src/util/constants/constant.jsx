export const HEADER_MENU = [
  {
    id: 0,
    gnbmenu: "Outdoor",
    lnbmenu: [
      { id: 1, submenu: "체어", conponent: "OutdoorChairPage", url: "/Helinox-react/outdoorchair", tab: true, count: 2 },
      { id: 2, submenu: "코트", url: "/Helinox-react/outdoorcort", tab: false, count: 0 },
      { id: 3, submenu: "테이블", url: "#!", tab: false, count: 0 },
      { id: 4, submenu: "텐트", url: "#!", tab: false, count: 0 },
      { id: 5, submenu: "우산 및 기타", url: "#!", tab: false, count: 0 },
      { id: 6, submenu: "도그코트", url: "#!", tab: false, count: 0 },
      { id: 7, submenu: "스토리지", url: "#!", tab: false, count: 0 },
    ],
  },
  {
    id: 1,
    gnbmenu: "Home",
    lnbmenu: [
      { id: 1, submenu: "체어", url: "#!", tab: true, count: 0 },
      { id: 2, submenu: "코트", url: "#!", tab: false, count: 0 },
      { id: 3, submenu: "테이블", url: "#!", tab: false, count: 0 },
    ],
  },
  {
    id: 2,
    gnbmenu: "Tactical",
    lnbmenu: [
      { id: 1, submenu: "체어", url: "#!", tab: true, count: 0 },
      { id: 2, submenu: "테이블", url: "#!", tab: false, count: 0 },
      { id: 3, submenu: "텍티컬 필드 오피스", url: "#!", tab: false, count: 0 },
      { id: 4, submenu: "텐트", url: "#!", tab: false, count: 0 },
      { id: 5, submenu: "코트", url: "#!", tab: false, count: 0 },
      { id: 6, submenu: "사이드 스트리지 및 우산", url: "#!", tab: false, count: 0 },
      { id: 7, submenu: "텍티컬 어드벤스드 스킨", url: "#!", tab: false, count: 0 },
    ],
  },
  {
    id: 3,
    gnbmenu: "Accessories",
    lnbmenu: [
      { id: 1, submenu: "택티컬 필드 오피스 M 악세서리", url: "#!", tab: true, count: 0 },
      { id: 2, submenu: "체어 악세서리", url: "#!", tab: false, count: 0 },
      { id: 3, submenu: "워머류", url: "#!", tab: false, count: 0 },
      { id: 4, submenu: "테이블 악세서리", url: "#!", tab: false, count: 0 },
      { id: 5, submenu: "코트 악세서리", url: "#!", tab: false, count: 0 },
      { id: 6, submenu: "텐트 악세서리", url: "#!", tab: false, count: 0 },
      { id: 7, submenu: "스틱 악세서리", url: "#!", tab: false, count: 0 },
      { id: 8, submenu: "ETC", url: "" },
    ],
  },
  {
    id: 4,
    gnbmenu: "Special/Limited",
    lnbmenu: [
      { id: 1, submenu: "Leica x Helinox", url: "#!", tab: true, count: 0 },
      { id: 2, submenu: "BTS | Helinox", url: "#!", tab: false, count: 0 },
      { id: 3, submenu: "퍼니쳐", url: "#!", tab: false, count: 0 },
      { id: 4, submenu: "기타", url: "#!, tab: false" },
      { id: 5, submenu: "포인트 상품", url: "#!", tab: false, count: 0 },
      { id: 6, submenu: "헬리녹스 인사이더즈", url: "#!", tab: false, count: 0 },
      { id: 7, submenu: "10주년", url: "#!", tab: false, count: 0 },
      { id: 8, submenu: "아카이브", url: "#!", tab: false, count: 0 },
      { id: 9, submenu: "Special SKU", url: "#!", tab: false, count: 0 },
    ],
  },
  {
    id: 5,
    gnbmenu: "T.E.R.G",
    lnbmenu: [
      { id: 1, submenu: "백팩&멀티백", url: "#!", tab: true, count: 0 },
      { id: 2, submenu: "크로스백&웨이스트백", url: "#!", tab: false, count: 0 },
      { id: 3, submenu: "울트라 라이트", url: "#!", tab: false, count: 0 },
      { id: 4, submenu: "파우치&ACC", url: "#!", tab: false, count: 0 },
      { id: 5, submenu: "멀티링크", url: "#!", tab: false, count: 0 },
      { id: 6, submenu: "멀티베스트", url: "#!", tab: false, count: 0 },
    ],
  },
  {
    id: 6,
    gnbmenu: "+Information",
    lnbmenu: [
      { id: 1, submenu: "Our story", url: "#!", tab: true, count: 0 },
      { id: 2, submenu: "공지사항", url: "#!", tab: false, count: 0 },
      { id: 3, submenu: "자주 묻는 질문", url: "#!", tab: false, count: 0 },
      { id: 4, submenu: "상품 문의", url: "#!", tab: false, count: 0 },
      { id: 5, submenu: "이벤트", url: "#!", tab: false, count: 0 },
      { id: 6, submenu: "A/S", url: "#!", tab: false, count: 0 },
      { id: 7, submenu: "HCC", url: "#!", tab: false, count: 0 },
    ],
  },
];

export const FILTER_CHECK = [
  {
    id: 0,
  },
];

export const FILTER_PRODUCT_TYPE = [
  {
    id: 0,
    type: "체어원",
  },
  {
    id: 1,
    type: "체어투",
  },
  {
    id: 2,
    type: "선셋체어",
  },
  {
    id: 3,
    type: "비치체어",
  },
  {
    id: 4,
    type: "사바나",
  },
  {
    id: 5,
    type: "플라야",
  },
  {
    id: 6,
    type: "그라운드",
  },
  {
    id: 7,
    type: "스위블",
  },
  {
    id: 8,
    type: "페스티벌",
  },
  {
    id: 9,
    type: "벤치원",
  },
  {
    id: 10,
    type: "스피드 스툴",
  },
  {
    id: 11,
    type: "카페체어",
  },
];

export const FILTER_COLOR = [
  {
    id: 0,
    color: "black",
  },
  {
    id: 1,
    color: "white",
  },
  {
    id: 2,
    color: "gray",
  },
  {
    id: 3,
    color: "burlywood",
  },
  {
    id: 4,
    color: "tan",
  },
  {
    id: 5,
    color: "red",
  },
  {
    id: 6,
    color: "blue",
  },
  {
    id: 7,
    color: "green",
  },
  {
    id: 8,
    color: "yellow",
  },
];

export const FILTER_SIZE = [
  {
    id: 0,
    size: "S",
  },
  {
    id: 1,
    size: "M",
  },
  {
    id: 2,
    size: "L",
  },
  {
    id: 3,
    size: "XL",
  },
];
